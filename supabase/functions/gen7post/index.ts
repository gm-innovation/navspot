...
const V="7.9.12";
function extractSourceBody(script:string):string{const lines=script.split("\n");let inSource=false;const body:string[]=[];for(const line of lines){if(!inSource){if(line.endsWith('source="')){inSource=true;continue;}continue;}if(line==='"'){break;}body.push(line);}return body.join("\n");}
