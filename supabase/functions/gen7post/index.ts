...
if(body.mode==="serve"){
  ...
  // sync types — all serve the full template directly
  if(type==="sync-standalone"||type==="sync-source"||type==="sync-rsc"){
    const full=await tpl("sync-standalone",v);
    return new Response(full,{headers:{...H,"Content-Type":"text/plain; charset=utf-8"}});
  }
  // guardian types — all serve the full template directly
  if(type==="guardian-standalone"||type==="guardian-source"||type==="guardian-rsc"){
    const full=await tpl("guardian-standalone",v);
    return new Response(full,{headers:{...H,"Content-Type":"text/plain; charset=utf-8"}});
  }
  if(type==="all"){
    const s0=await tpl("infra",v);
    const s1=await tpl("sync-standalone",v);
    const s2=await tpl("guardian-standalone",v);
